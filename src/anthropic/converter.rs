...
pub(crate) const KIRO_OPUS_4_7: &str = "claude-opus-4.7";
pub(crate) const KIRO_OPUS_4_6: &str = "claude-opus-4.6";
pub(crate) const KIRO_OPUS_4_5: &str = "claude-opus-4.5";
pub(crate) const KIRO_SONNET_4_6: &str = "claude-sonnet-4.6";
pub(crate) const KIRO_SONNET_4_5: &str = "claude-sonnet-4.5";
pub(crate) const KIRO_SONNET_4: &str = "claude-sonnet-4";
pub(crate) const KIRO_HAIKU_4_5: &str = "claude-haiku-4.5";

fn normalize_model_name(model: &str) -> String {
    model.to_lowercase().replace('.', "-").replace('_', "-")
}

fn has_family_version(model_key: &str, family: &str, version: &str) -> bool {
    let version_key = version.replace('.', "-");
    let pattern = format!("{family}-{version_key}");
    let mut offset = 0;

    while let Some(relative_idx) = model_key[offset..].find(&pattern) {
        let end = offset + relative_idx + pattern.len();
        let has_boundary = model_key[end..]
            .chars()
            .next()
...
fn has_plain_family_major(model_key: &str, family: &str, major: &str) -> bool {
    let pattern = format!("{family}-{major}");
    let mut offset = 0;

    while let Some(relative_idx) = model_key[offset..].find(&pattern) {
        let end = offset + relative_idx + pattern.len();
        let suffix = &model_key[end..];

        if suffix.is_empty() || suffix.starts_with("-thinking") || suffix.starts_with("-20") {
            return true;
        }
...
///
/// 按照用户要求：
/// - opus 4.7/4-7 或未指定版本 → claude-opus-4.7
/// - opus 4.6/4-6 → claude-opus-4.6
/// - opus 4.5/4-5 → claude-opus-4.5
/// - sonnet 4.6/4-6 或未指定版本 → claude-sonnet-4.6
...
pub fn map_model(model: &str) -> Option<String> {
    ...
    if model_key.contains("sonnet") {
        if has_family_version(&model_key, "sonnet", "4.6") {
            Some(KIRO_SONNET_4_6.to_string())
        } else if has_family_version(&model_key, "sonnet", "4.5") {
            Some(KIRO_SONNET_4_5.to_string())
        } else if has_plain_family_major(&model_key, "sonnet", "4") {
            Some(KIRO_SONNET_4.to_string())
        } else {
            Some(KIRO_SONNET_4_6.to_string())
        }
    } else if model_key.contains("opus") {
        if has_family_version(&model_key, "opus", "4.7") {
            Some(KIRO_OPUS_4_7.to_string())
        } else if has_family_version(&model_key, "opus", "4.6") {
            Some(KIRO_OPUS_4_6.to_string())
        } else if has_family_version(&model_key, "opus", "4.5") {
            Some(KIRO_OPUS_4_5.to_string())
        } else {
            Some(KIRO_OPUS_4_7.to_string())
        }
    } else if model_key.contains("haiku") {
        Some(KIRO_HAIKU_4_5.to_string())
    } else {
        None
    }
}
...
/// Opus 4.7、Opus 4.6 和 Sonnet 4.6 使用 1M 上下文。
pub fn get_context_window_size(model: &str) -> i32 {
    if let Some(mapped) = map_model(model) {
        let is_million_context =
            [KIRO_OPUS_4_7, KIRO_OPUS_4_6, KIRO_SONNET_4_6].contains(&mapped.as_str());
...
mod tests {
    ...
    fn test_map_model_thinking_suffix_sonnet() {
        ...
        assert_eq!(
            map_model("claude-sonnet-4-thinking"),
            Some(KIRO_SONNET_4.to_string())
        );
        assert_eq!(
            map_model("claude-sonnet-4-20250514-thinking"),
            Some(KIRO_SONNET_4.to_string())
        );
    }
    ...
    fn test_map_model_thinking_suffix_opus_4_7() {
        ...
        assert_eq!(result, Some(KIRO_OPUS_4_7.to_string()));
    }
    ...
    fn test_map_model_thinking_suffix_opus_4_5() {
        ...
        assert_eq!(result, Some(KIRO_OPUS_4_5.to_string()));
    }
    ...
    fn test_map_model_thinking_suffix_opus_4_6() {
        ...
        assert_eq!(result, Some(KIRO_OPUS_4_6.to_string()));
    }
    ...
    fn test_map_model_thinking_suffix_haiku() {
        ...
        assert_eq!(result, Some(KIRO_HAIKU_4_5.to_string()));
    }

    #[test]
    fn test_map_model_latest_kiro_models() {
        assert_eq!(map_model(KIRO_OPUS_4_7), Some(KIRO_OPUS_4_7.to_string()));
        assert_eq!(map_model(KIRO_OPUS_4_6), Some(KIRO_OPUS_4_6.to_string()));
        assert_eq!(
            map_model(KIRO_SONNET_4_6),
            Some(KIRO_SONNET_4_6.to_string())
        );
        assert_eq!(map_model(KIRO_OPUS_4_5), Some(KIRO_OPUS_4_5.to_string()));
        assert_eq!(
            map_model(KIRO_SONNET_4_5),
            Some(KIRO_SONNET_4_5.to_string())
        );
        assert_eq!(map_model(KIRO_SONNET_4), Some(KIRO_SONNET_4.to_string()));
        assert_eq!(map_model(KIRO_HAIKU_4_5), Some(KIRO_HAIKU_4_5.to_string()));
    }

    #[test]
    fn test_get_context_window_size() {
        assert_eq!(get_context_window_size(KIRO_OPUS_4_7), 1_000_000);
        assert_eq!(get_context_window_size(KIRO_OPUS_4_6), 1_000_000);
        assert_eq!(get_context_window_size(KIRO_SONNET_4_6), 1_000_000);
        assert_eq!(get_context_window_size(KIRO_OPUS_4_5), 200_000);
        assert_eq!(get_context_window_size(KIRO_SONNET_4), 200_000);
    }
